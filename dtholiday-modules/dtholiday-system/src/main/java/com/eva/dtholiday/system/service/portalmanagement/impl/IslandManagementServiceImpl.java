...
public class IslandManagementServiceImpl implements IslandManagementService {
    ...
    public ResponseApi islandManagementAdd(IslandManagementReq islandManagementReq) {
        ...
        // 重名校验
        IslandManagement oldIsland = islandManagementMapper.selectOne(new QueryWrapper<IslandManagement>().lambda()
            .eq(IslandManagement::getIslandCnName, islandManagementReq.getIslandCnName()));
        if (oldIsland != null) {
            throw new BusinessException(BusinessErrorCodeEnum.ISLAND_NAME_HAS_EXISTED.getMessageCN(),
                BusinessErrorCodeEnum.ISLAND_NAME_HAS_EXISTED.getCode());
        }
        // 插入主表
        ...
        if (CollectionUtils.isNotEmpty(islandManagementReq.getIslandQuotationPdfList())) {
            List<IslandQuotation> islandQuotationList = convertQuotationEntityList(
                islandManagementReq.getIslandQuotationPdfList(), islandManagement.getIslandIndexCode());
    ...
    private List<IslandQuotation> convertQuotationEntityList(List<IslandQuotation> islandQuotationPdfList,
        Integer islandIndexCode) {
        List<IslandQuotation> list = new ArrayList<>();
        islandQuotationPdfList.forEach(item -> {
    ...
    public ResponseApi islandManagementUpdate(IslandManagementReq islandManagementReq) {
        // 岛屿存在性校验
        IslandManagement islandManagement = islandManagementMapper.selectOne(new QueryWrapper<IslandManagement>()
            .lambda().eq(IslandManagement::getIslandIndexCode, islandManagementReq.getIslandIndexCode()));
        if (islandManagement == null) {
            throw new BusinessException(BusinessErrorCodeEnum.ISLAND_NOT_EXISTED.getMessageCN(),
                BusinessErrorCodeEnum.ISLAND_NOT_EXISTED.getCode());
        }
        IslandManagement oldNameIsland = islandManagementMapper.selectOne(new QueryWrapper<IslandManagement>().lambda()
            .eq(IslandManagement::getIslandCnName, islandManagementReq.getIslandCnName()));
        // 更新时重名校验
        ...
        handleQuotationList(oldQuotationList, islandManagementReq.getIslandQuotationPdfList(), needAddList,
            needDeleteList);
        // 删除报价单
        List<Integer> batchDeleteIdList =
            needDeleteList.stream().map(IslandQuotation::getQuotationIndexCode).collect(Collectors.toList());
        if (CollectionUtils.isNotEmpty(batchDeleteIdList)) {
            islandQuotationMapper.deleteBatchIds(batchDeleteIdList);
        }
        // 新增报价单
        List<IslandQuotation> islandQuotationList =
            convertQuotationEntityList(needAddList, islandManagementReq.getIslandIndexCode());
        if (CollectionUtils.isNotEmpty(islandQuotationList)) {
            islandQuotationMapper.insertBatch(islandQuotationList);
        }
        LocalCache.putIslandName(islandManagement.getIslandIndexCode(), islandManagement.getIslandCnName());
        return islandManagementQueryDetail(islandManagement.getIslandIndexCode());
    ...
    public ResponseApi islandManagementQueryList(IslandManagementReq req) {
        ...
        if (CollectionUtils.isNotEmpty(req.getIslandTagList())) {
            ...
            if (CollectionUtils.isNotEmpty(islandTagRelationList)) {
                List<Integer> islandIndexCodeList = islandTagRelationList.stream().distinct()
                    .map(IslandTagRelation::getIslandIndexCode).collect(Collectors.toList());
        ...
        List<IslandManagementTagInfo> islandManagementTagInfoList = entityPage.getRecords().stream().map(entity -> {
            ...
            List<IslandTagRelation> islandTagRelationList =
                islandTagRelationMapper.selectList(new QueryWrapper<IslandTagRelation>()
                    .in(IslandTagRelation.ISLAND_INDEX_CODE, islandManagementTagInfo.getIslandIndexCode()));
            if (CollectionUtils.isNotEmpty(islandTagRelationList)) {
                List<Integer> tagIndexCodeList = islandTagRelationList.stream().distinct()
